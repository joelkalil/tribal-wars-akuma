...
var scriptConfig = {
    ...
    translations: {
        ...
        fr_FR: {
            ...
            'This script requires Premium Account to be active!':
                "Ce script nécessite un compte premium d'activé!",
            'Import Dynamic Group Configuration':
            ...
            'Import a group configuration then try again!':
                'Importez une configuration de groupe, puis réessayez!',
            'Invalid input!': 'Entrée invalide!',
            'Update Preview': 'Aperçu de la mise à jour',
            'Preview has been updated!': "L'aperçu a été mis à jour!",
            'Filters have been applied!': 'Les filtres ont été appliqués!',
            'Filters have already been applied!':
                'Les filtres ont déjà été appliqués!',
        },
    },
    allowedMarkets: [],
    ...
    function handleImportGroup() {
        jQuery('#raImportGroupBtn').on('click', function (e) {
            ...
            if (importedString.length === 0) {
                ...
                const jsonStringImportedData =
                    JSON.stringify(filteredImportedData);
                const jsonStringCurrentData = JSON.stringify(
                    filteredCurrentFormData
                );

                if (DEBUG) {
                    console.debug(
                        `${scriptInfo} importedString`,
                        importedString
                    );
                    console.debug(
                        `${scriptInfo} importedFormData`,
                        importedFormData
                    );
                    console.debug(
                ...
                if (Object.entries(filteredImportedData).length > 1) {
                    if (jsonStringImportedData !== jsonStringCurrentData) {
                        jQuery('#raUpdatePreviewBtn').removeClass(
                            'btn-disabled'
                        );
                        jQuery('html,body').animate(
                            {
                                scrollTop:
                                    jQuery('#filter_config').offset().top - 100,
                            },
                            'slow'
                        );
                        ...
                        for (let [key, value] of Object.entries(
                            ...
                            if (value === 'on') {
                                ...
                                setTimeout(function () {
                                    ...
                                    if (
                                        jQuery(`select[name="${key}"]`).length
                                    ) {
                                        jQuery(`select[name="${key}"]`).trigger(
                                            'change'
                                        );
                                        index++;
                                    }
                        ...
                        UI.ErrorMessage(
                            twSDK.tt('Filters have already been applied!')
                        );
                    }
                } else {
