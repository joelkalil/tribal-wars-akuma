/*
 * Script Name: Find Villages in Range
...
var MAX_COMBINATIONS_TO_SHOW = 120; // how many rows of combinations to show and export (recommended value is 120)

// Globals
if (typeof TWMap === 'undefined') TWMap = {};
const { villages, players, allies } = TWMap;

// Script Config
var scriptConfig = {
    scriptData: {
        prefix: 'findVillagesInRange',
        name: 'Find Villages in Range',
        version: 'v1.1.1-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/find-villages-in-range.287098/',
    },
    translations: {
        en_DK: {
            'Find Villages in Range': 'Find Villages in Range',
            Help: 'Help',
            ...
            Tribe: 'Tribe',
            'Chose Tribe': 'Chose Tribe',
            'All tribes': 'All tribes',
            'Get Mailing List': 'Get Mailing List',
            'Mailing List': 'Mailing List',
            'Mailing list is empty!': 'Mailing list is empty!',
            'There was an error while fetching the data!':
                'There was an error while fetching the data!',
        },
        fr_FR: {
            'Find Villages in Range': 'Trouver des villages dans une zone',
            Help: 'Aide',
            'Redirecting...': 'Redirection...',
            'Landing Time': 'Temps de trajet',
            Sigil: 'Sceau',
            'Calculate Launch Times': "Calculateur heure d'envoi",
            'Destination Village': 'Village de destination',
            'No possible combinations found!': 'Aucune combinaison trouvé!',
            From: 'Origine',
            Unit: 'Unité',
            Distance: 'Distance',
            ...
            'Send in': 'Envoi dans',
            Player: 'Joueur',
            'Export as BB Code': 'Exporté en BB Code',
            'Nothing to export!': 'Rien à exporté!',
            'Target:': 'Cible:',
            'Landing Time:': 'Temps de trajet:',
            'Choose Units': 'Choisir des unités',
            Tribe: 'Tribu',
            'Chose Tribe': 'Choisir une tribu',
            'All tribes': 'Toutes les tribus',
            'Get Mailing List': 'Récupérer Liste de diffusion',
            'Mailing List': 'Liste de diffusion',
            'Mailing list is empty!': 'Liste de diffusion vide!',
            'There was an error while fetching the data!':
                'There was an error while fetching the data!',
        },
    },
...
(async function () {
    ...
    const { worldUnitInfo } = await fetchWorldConfigData();

    // Entry Point
    (function () {
        if (isValidScreen) {
            initFindVillagesInRange();
        } else {
    ...
    function initFindVillagesInRange() {
        // run on script load
        const unitsTable = buildUnitsChoserTable();
        const tribesFilter = buildTribesChoser();
        const content = prepareContent(unitsTable, tribesFilter);

        const customStyle = `
                .ra-grid { display: grid; grid-template-columns: 180px 1fr 180px 180px; grid-gap: 0 20px; }
    ...
    // Helper: Prepare UI
    function prepareContent(unitsTable, tribesFilter) {
        return `
                <div class="ra-mb15">
                    <div class="ra-grid">
                        <div>
                            <label for="raDestinationVillage">${twSDK.tt(
                                'Destination Village'
                            )}</label>
                            <input readonly type="text" id="raDestinationVillage" value="${
                                TWMap.pos[0].toString() +
                                '|' +
                                TWMap.pos[1].toString()
                            }" class="ra-input">
                        </div>
                        <div>
                            ...
                            <input id="raLandingTime" type="text" class="ra-input" value="${new Date()
                                .toLocaleString('en-GB')
                                .replace(',', '')}">
                        </div>
                        <div>
                            <label for="raSigil">${twSDK.tt('Sigil')}</label>
                            <input id="raSigil" type="text" value="0" class="ra-input">
                        </div>
                        <div>
                            <label for="raSigil">${twSDK.tt(
                                'Chose Tribe'
                            )}</label>
                            ${tribesFilter}
                        </div>
                ...
                <div class="ra-mb15">
                    <label>${twSDK.tt('Choose Units')}</label>
                    ${unitsTable}
                </div>
                <div>
                    <a href="javascript:void(0);" id="calculateLaunchTimesBtn" class="btn btn-confirm-yes">
                        ${twSDK.tt('Calculate Launch Times')}
                    </a>
                    <a href="javascript:void(0);" id="exportBBCodeBtn" class="btn">
                        ${twSDK.tt('Export as BB Code')}
                    </a>
                    <a href="javascript:void(0);" id="mailingListBtn" class="btn">
                        ${twSDK.tt('Get Mailing List')}
                    </a>
                </div>
                <div style="display:none;" class="ra-mt15" id="raPossibleCombinations">
            ...
            if (unit !== 'spy' && unit !== 'militia') {
                ...
                tableRow += `
                        <td class="ra-text-center">
                            <input name="ra_chosen_units" type="checkbox" id="unit_${unit}" class="ra-unit-selector" value="${unit}" />
                        </td>
                    `;
            }
        });
    ...
    function calculateLaunchTimes() {
        jQuery('#calculateLaunchTimesBtn').on('click', function (e) {
            e.preventDefault();

            // collect user input and destination village
            const destinationVillage = jQuery('#raDestinationVillage')
                .val()
                .trim();
            const landingTimeString = jQuery('#raLandingTime').val().trim();
            const sigil = jQuery('#raSigil').val().trim();
            const chosenTribe = jQuery('#raTribesChoser').val();
            ...
            for (let [key, village] of Object.entries(villages)) {
                if (village.owner !== '0' && village.owner !== 0) {
                    const coords = splitString(key);
                    const { id, name, owner } = village;
                    ...
                    chosenUnits.forEach((unit) => {
                        ...
                        if (launchTime > serverTime.getTime()) {
                            ...
                            if (distance > 0) {
                                if (parseInt(chosenTribe) !== 0) {
                                    if (
                                        parseInt(chosenTribe) ===
                                        parseInt(villageOwner.ally)
                                    ) {
                                        possibleSnipes.push({
                                            id: id,
                                            name: name,
            ...
            possibleSnipes = possibleSnipes.slice(0, MAX_COMBINATIONS_TO_SHOW);
            ...
            if (possibleSnipes.length > 0) {
                ...
                jQuery('#possibleCombinationsTable').html(
                    snipeCombinationsTable
                );
                jQuery('#exportBBCodeBtn').attr(
                    'data-snipe',
                    JSON.stringify(possibleSnipes)
                );
                jQuery('#mailingListBtn').attr(
                    'data-players',
                    JSON.stringify(possibleSnipes)
                );
                Timing.tickHandlers.timers.init();
            } else {
                UI.ErrorMessage(twSDK.tt('No possible combinations found!'));
                jQuery('#raPossibleCombinations').hide();
                jQuery('#possibleCombinationsTable').html('');
                jQuery('#exportBBCodeBtn').attr('data-snipe', '');
                jQuery('#mailingListBtn').attr('data-players', '');
            }
        });
    }
            ...
            if (snipeAttempts) {
                ...
                const content = `
                        <div class="ra-popup-content">
                            <label for="exportBBCodeInput">${twSDK.tt(
                                'Export as BB Code'
                            )}</label>
    ...
    // Action Handler: Generate Mailing List
    function generateMailingList() {
        jQuery('#mailingListBtn').on('click', function (e) {
            e.preventDefault();
            const players = jQuery(this).attr('data-players');
            if (players) {
                jQuery(this).addClass('btn-confirm-yes');
                const playersJSON = JSON.parse(players);
                let playersArray = playersJSON.map(
                    (player) => player.owner.name
                );
                playersArray = [...new Set(playersArray)];
                const mailingList = playersArray.join(';');
                const content = `
                        <div class="ra-popup-content">
                            <label for="exportMailingList">${twSDK.tt(
                                'Mailing List'
                            )}</label>
                            <textarea readonly id="exportMailingList">${mailingList}</textarea>
                        </div>
                    `;
                Dialog.show('content', content);
            } else {
                UI.ErrorMessage(twSDK.tt('Mailing list is empty!'));
            }
        });
    }
    ...
    function getBBCodeExport(snipes) {
        ...
        bbCode += `[table][**]${twSDK.tt('Unit')}[||]${twSDK.tt(
            'From'
        )}[||]${twSDK.tt('Launch Time')}[||]${twSDK.tt('Player')}[/**]\n`;

        snipes.forEach((plan) => {
            const { coords, formattedLaunchTime, unit, owner } = plan;
            const playerLink = `/game.php?screen=info_player&id=${owner.owner}`;
            bbCode += `[*][unit]${unit}[/unit][|] ${coords} [|]${formattedLaunchTime}[|][url=${window.location.origin}${playerLink}]${owner.name}[/url]\n`;
        });
    ...
    function buildCombinationsTable(snipes) {
        ...
        snipes.forEach((snipe, index) => {
            ...
            const tribeInfo = allies[owner.ally];
            const continent = twSDK.getContinentByCoord(coords);
            ...
            combinationsTable += `
                    <tr>
                        ...
                        <td class="ra-text-left">
                            <a href="/game.php?screen=info_player&id=${
                                owner.owner
                            }" target="_blank" rel="noopener noreferrer">
                                ${owner.owner !== '0' ? owner.name : ''}
                            </a>
                        </td>
                        <td class="ra-tac">
                            <a href="/game.php?screen=info_ally&id=${
                                owner.ally
                            }" target="_blank" rel="noopener noreferrer">
                                ${owner.ally !== '0' ? tribeInfo?.tag : ''}
                            </a>
                        </td>
                        <td class="ra-tac">
                            ${formattedLaunchTime}
    ...
    // Render Tribes Choser table
    function buildTribesChoser() {
        let tribesFilter = `<select id="raTribesChoser" class="ra-input"><option value="0">${twSDK.tt(
            'All tribes'
        )}</option>`;
        for (let [key, tribe] of Object.entries(allies)) {
            tribesFilter += `<option value="${key}">${tribe.tag}</option>`;
        }
        tribesFilter += `</select>`;
        return tribesFilter;
    }

    // Helper: Split string in half to get coords
