...
(async function () {
    ...
    async function initMain() {
        try {
            ...
            handleSelectNotes({ players, tribes });
            handleResetFilters({ players, tribes });
        } catch (error) {
    ...
    function handleSelectNotes(data) {
        jQuery('#raSelectNotesBtn').on('click', function (e) {
            e.preventDefault();

            const { players, tribes } = data;
            const { playersInput, tribesInput, excludedPlayersInput } =
                collectUserInput();
            ...
            let chosenPlayers = playersInput.split(',');
            let chosenTribes = tribesInput.split(',');
            let chosenExcludedPlayers = excludedPlayersInput.split(',');

            let chosenPlayerIds = twSDK.getEntityIdsByArrayIndex(
                chosenPlayers,
                players,
                1
            );
            ...
            if (playersInput === '--Barbarian--') {
                chosenPlayerIds = [0];
            }

            const tribePlayers = getTribeMembersById(chosenTribeIds, players);
            ...
            jQuery('form:eq(1) > table > tbody > tr').each(function () {
                const playerId = parseInt(
                    jQuery(this).find('span.village_anchor').attr('data-player')
                );

                if (uniquePlayersList.includes(playerId)) {
                    numberOfNotesSelected++;
                    jQuery(this)
                        .find('td:eq(3) input[type="checkbox"]')
                        .prop('checked', true);
                    jQuery(this).show();
                } else {
                    jQuery(this)
            ...
            if (numberOfNotesSelected > 0) {
                const message =
                    numberOfNotesSelected +
                    ' ' +
                    twSDK.tt('notes have been selected!');
    ...
    function buildUI(data) {
        ...
        twSDK.renderBoxWidget(
            contentBody,
            'raOwnNotesManager',
            'ra-own-notes-manager',
            customStyle
        );
    }

    // Render: Prepare the content
    ...
    function buildDropDown(array, entity) {
        ...
        if (entity === 'Players') {
            dropdown += `<option value="${twSDK.tt('--Barbarian--')}">`;
        }
    ...
    function getTribeMembersById(tribeIds, players) {
        const tribeMemberIds = [];
        players.forEach((player) => {
            if (tribeIds.includes(parseInt(player[2]))) {
                tribeMemberIds.push(parseInt(player[0]));
    ...
    function collectUserInput() {
        const playersInput = jQuery('#raPlayers').val();
        const tribesInput = jQuery('#raTribes').val();
        const excludedPlayersInput = jQuery('#raExcludedPlayers').val();
