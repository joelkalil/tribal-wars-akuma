...
var scriptConfig = {
    ...
    translations: {
        ...
        hu_HU: {
            'Defense Health Check': 'Védelem levizsgáló',
            Help: 'Segítség',
            'Redirecting...': 'Átírányítás...',
            'This script needs to be run on village overview!':
                'A scriptet a falu áttekintésnél kell futtatni!',
            'There was an error fetching troops stationed on your village!':
                'Valami hiba jött fel a faluban lévő sereg kiolvasása közben!',
            'There was an error fetching simulation data!':
                'Hiba keletkezett a szimuláció létrehozása közben!',
            'Check Stack Health': 'Sereg ellenőrzése',
            'Take into account incoming support':
                'Bejövő erősítések figyelembe vétele.',
            'Invalid input!': 'Érvénytelen bemenetel!',
            '[SIM]': '[SZIM]',
            'more nuke like this is killed.': 'hasonló nuke megfogva.',
            'more nukes like this are killed.': 'hasonló nuke megfogva!',
            'No nuke is killed!': 'Egyetlen egy nuket sem fog meg!',
            'Enable night bonus': 'Éjszakai bónusz figyelembe vétele',
            'Run the script again on village overview!':
                'Run the script again on village overview!',
            Reset: 'Reset',
            'Configuration was resetted!': 'Configuration was resetted!',
...
(async function () {
    ...
    (function () {
        if (isValidScreen) {
            ...
            if (gameScreen === 'place' && gameMode === 'sim') {
                $.getScript(
                    'https://raw.githubusercontent.com/joelkalil/tribal-wars-akuma/refs/heads/main/src/scripts/barra_de_acesso/fillTroopsInSimulator.js'
                );
                ...
                units.marcher = Number(row.find('.unit-item-marcher').text());
                units.heavy = Number(row.find('.unit-item-heavy').text());
                units.ram = Number(row.find('.unit-item-ram').text());
                units.catapult = Number(row.find('.unit-item-catapult').text());
    ...
    function buildUI() {
        const unitAmounts =
            JSON.parse(
                localStorage.getItem(`${scriptConfig.scriptData.prefix}_data`)
            ) || DEFAULT_NUKE;
            ...
            if (simulationResults) {
                if (simulationResults > 10) {
                    content = `
                                <div class="info_box ra-alert-box ra-success">
                                    <img src="/graphic/stat/green.webp"> <b>${simulationResults}</b> ${twSDK.tt(
                        'more nukes like this are killed.'
                    )}</b>
                    ...
                    content = `
                                <div class="info_box ra-alert-box">
                                    <img src="/graphic/stat/yellow.webp"> <b>${simulationResults}</b> ${twSDK.tt(
                        simulationResults === 1
                ...
                content = `
                            <div class="info_box ra-alert-box ra-danger">
                                <img src="/graphic/stat/red.webp"> <b>${twSDK.tt(
                                    'No nuke is killed!'
                                )}</b>
                                <a href="${simulationUrl}" target="_blank" rel="noreferrer">
                                    ${twSDK.tt('[SIM]')}
    ...
    function calculateCurrentStack() {
        ...
        let nukes =
            Math.round((0.205 * defCount * defCount + 1.079 * defCount) * 100) /
            100;
