...
(async function () {
    ...
    function buildUI() {
        const { landingTime, unitsToSend, sigil, selectedUnit, coordinates } =
            initDefaultValues();
            ...
            ownVillages.forEach((village) => {
                ...
                coordinates.split(' ').forEach((coord) => {
                    ...
                    if (
                        ...
                        const travelTime = twSDK.secondsToHms(unformattedTime);
    ...
    function handleResetScript() {
        jQuery('#raResetScriptBtn').on('click', function (e) {
            ...
            localStorage.removeItem(`${scriptConfig.scriptData.prefix}_config`);
    ...
    function handleOpenMultipleTabs() {
        jQuery('#raOpenTabsBtn').on('click', function (e) {
            ...
            if (isUK) {
                UI.ErrorMessage(
                    twSDK.tt('This feature is not available in the UK server!')
                );
                return;
            }
            ...
            jQuery('#combinationsTable tbody tr').each(function (index, row) {
                if (index < 25) {
                    setTimeout(function () {
    ...
    function handleExport() {
        jQuery('#raExportBtn').on('click', function (e) {
            ...
            let commands = JSON.parse(jQuery(this).attr('data-commands')) ?? [];
    ...
    function initDefaultValues() {
        const config =
            JSON.parse(
                localStorage.getItem(`${scriptConfig.scriptData.prefix}_config`)
            ) ?? {};
