/*
 * Script Name: Commands Overview
 * Version: v1.0.4-akuma.1
...
var scriptConfig = {
    scriptData: {
        prefix: 'commandsOverview',
        name: 'Commands Overview',
        version: 'v1.0.3-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/commands-overview.287538/',
    },
    translations: {
        en_DK: {
            ...
            Return: 'Return',
            Support: 'Support',
            'Farming runs': 'Farming runs',
            'Returning farming run': 'Returning farming run',
            'Attack small': 'Attack small',
            'Attack medium': 'Attack medium',
            'Attack large': 'Attack large',
            'Returning attack small': 'Returning attack small',
            'Returning attack medium': 'Returning attack medium',
            'Returning attack large': 'Returning attack large',
            Filter: 'Filter',
            Type: 'Type',
            Action: 'Action',
            'Commands have been filtered!': 'Commands have been filtered!',
            'No command found for this filter!':
                'No command found for this filter!',
            'Reset Filters': 'Reset Filters',
            'All filter commands have been cleared!':
                'All filter commands have been cleared!',
            'Commands with Nobles': 'Commands with Nobles',
            'Show Combinations': 'Show Combinations',
            'There was an error collecting command informations!':
                'There was an error collecting command informations!',
            'Sent back': 'Sent back',
        },
    },
...
(async function () {
    ...
    if (isValidScreen && isValidMode) {
        ...
        twSDK.redirectTo('overview_villages&mode=commands');
        return;
    }

    // Initialize main script logic
    function initCommandsParser() {
        const commands = collectCommands();

        const {
            destinationVillagesCountArray,
            originVillagesCountArray,
            destinations,
            origins,
            commandTypesCount,
            commandSubTypesCount,
            commandsWithNobles,
        } = parseCommands(commands);

        if (DEBUG) {
            console.debug(`${scriptInfo} commands`, commands);
            console.debug(
                `${scriptInfo} destinationVillagesCountArray`,
                destinationVillagesCountArray
            );
            console.debug(
                `${scriptInfo} originVillagesCountArray`,
                originVillagesCountArray
            );
            console.debug(`${scriptInfo} destinations`, destinations);
            console.debug(`${scriptInfo} origins`, origins);
            console.debug(`${scriptInfo} commandTypesCount`, commandTypesCount);
            console.debug(
                `${scriptInfo} commandSubTypesCount`,
                commandSubTypesCount
            );
            console.debug(
                `${scriptInfo} commandsWithNobles`,
                commandsWithNobles
            );
        }
        ...
        const customStyle = `
                ...
                .ra-h400 { max-height: 400px; }
                .ra-w768 { width: 768px; }
                .ra-overflow-auto { overflow: auto; }
                .ra-p p { margin: 5px 0; }
            `;
        ...
        onClickCommandFilter(commands);
        onClickResetAllFilters();
        onClickFilterNobles(commands);
        onClickShowCombinations(
            destinationVillagesCountArray,
            originVillagesCountArray
        );
        onClickCommandRenamer();
    }

    // Action Handler: Handle click on command filter button
    function onClickCommandFilter(commands) {
        jQuery('.ra-filter-commands').on('click', function (e) {
            e.preventDefault();

            const chosenFilter = jQuery(this).attr('data-action-filter');

            const remainingCommands = commands.filter((command) => {
                return command.subType === chosenFilter + '.webp';
            });

            if (remainingCommands.length) {
                // hide and show table row accordingly
                jQuery(`#commands_table tbody tr.nowrap`).hide();
                remainingCommands.forEach((command) => {
                    jQuery(
                        `#commands_table tbody tr.nowrap input[value=${command.commandId}]`
                    )
                        .parent()
                        .parent()
                        .show();
                ...
                // ui notifications
                jQuery('#raFilterNoblesBtn').removeClass('btn-confirm-yes');
                jQuery('#raResetAllFiltersBtn').removeClass('btn-confirm-yes');
                jQuery('.ra-filter-commands').removeClass('btn-confirm-yes');
                jQuery(this).addClass('btn-confirm-yes');
                UI.SuccessMessage(twSDK.tt('Commands have been filtered!'));
            } else {
                UI.ErrorMessage(twSDK.tt('No command found for this filter!'));
            }
        });
    }

    // Action Handler: Reset all filters
    function onClickResetAllFilters() {
        jQuery('#raResetAllFiltersBtn').on('click', function (e) {
            e.preventDefault();

            // unhide hidden table row
            jQuery('#commands_table tbody tr.nowrap').show();
            ...
            jQuery(this).addClass('btn-confirm-yes');
            UI.SuccessMessage(
                twSDK.tt('All filter commands have been cleared!')
            );
        });
    }

    // Action Handler: Handle click on command filter button
    function onClickFilterNobles(commands) {
        jQuery('#raFilterNoblesBtn').on('click', function (e) {
            e.preventDefault();

            const remainingCommands = commands.filter((command) => {
                return (
                    command.containsNoble === 'snob.webp' ||
                    command.containsNoble === 'return_snob.webp'
                );
            });
    ...
    // Action Handler: Reset all filters
    function onClickShowCombinations(
        destinationVillagesCountArray,
        originVillagesCountArray
    ) {
        jQuery('#raShowCombinationsBtn').on('click', function (e) {
            e.preventDefault();

            const destinationVillagesTable = buildTable(
                destinationVillagesCountArray,
                'destination'
            );
            const originVillagesTable = buildTable(
                originVillagesCountArray,
                'origin'
            );

            const content = `
                    <div class="ra-grid ra-w768">
                        <div class="ra-h400 ra-overflow-auto">
                            ${destinationVillagesTable}
                        </div>
                        <div class="ra-h400 ra-overflow-auto">
                            ${originVillagesTable}
                        </div>
            ...
            Dialog.show('destinationOriginCombinations', content);
        });
    }
    ...
    function prepareContent(data) {
        ...
        const content = `
                <div class="ra-mb15">
                    <div class="ra-grid">
                        <div class="ra-p">
                            <p><b>${twSDK.tt('Destination Villages')}:</b> ${
            destinations.length ?? 0
        }</p>
                            <p><b>${twSDK.tt('Origin Villages')}:</b> ${
            origins.length ?? 0
        }</p>
                            <p><b>${twSDK.tt('Attack')}:</b> ${
            commandTypesCount.attack ?? 0
        }</p>
                            <p><b>${twSDK.tt('Return')}:</b> ${
            commandTypesCount.return ?? 0
        }</p>
                            <p><b>${twSDK.tt('Support')}:</b> ${
            commandTypesCount.support ?? 0
        }</p>
                            <p><b>${twSDK.tt('Commands with Nobles')}:</b> ${
            commandsWithNobles.length ?? 0
        }</p>
                            <br>
                            <a href="javascript:void(0);" class="btn" id="raResetAllFiltersBtn">
                                ${twSDK.tt('Reset Filters')}
                            </a>
                            <a href="javascript:void(0);" class="btn" id="raFilterNoblesBtn">
                                ${twSDK.tt('Commands with Nobles')}
                            </a>
                            <a href="javascript:void(0);" class="btn" id="raShowCombinationsBtn">
                                ${twSDK.tt('Show Combinations')}
                            </a>
                        </div>
                        <div>
                            <table class="ra-table ra-table-v2" width="100%">
                                ...
                                <tbody>
                                    <tr>
                                        <td class="ra-tal"><b>${twSDK.tt(
                                            'Farming runs'
                                        )}</b></td>
                                        <td>${
                                            commandSubTypesCount['farm.webp'] ??
                                            0
                                        }</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="farm">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="return_farm">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="attack_small">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="attack_medium">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="attack_large">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="ra-tal"><b>${twSDK.tt(
                                            'Returning attack small'
                                        )}</b></td>
                                        <td>${
                                            commandSubTypesCount[
                                                'return_attack_small.webp'
                                            ] ?? 0
                                        }</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="return_attack_small">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="ra-tal"><b>${twSDK.tt(
                                            'Returning attack medium'
                                        )}</b></td>
                                        <td>${
                                            commandSubTypesCount[
                                                'return_attack_medium.webp'
                                            ] ?? 0
                                        }</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="return_attack_medium">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="ra-tal"><b>${twSDK.tt(
                                            'Returning attack large'
                                        )}</b></td>
                                        <td>${
                                            commandSubTypesCount[
                                                'return_attack_large.webp'
                                            ] ?? 0
                                        }</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="return_attack_large">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>${
                                            commandSubTypesCount['back.webp'] ??
                                            0
                                        }</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="back">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="btn ra-filter-commands" data-action-filter="support">
                                                ${twSDK.tt('Filter')}
                                            </a>
                                        </td>
                                    </tr>
                ...
                <div class="ra-grid">
                    <div>
                        <label for="raDestinationCoords">${twSDK.tt(
                            'Destination Villages'
                        )}: ${destinations.length}</label>
                        <textarea id="raDestinationCoords" class="ra-textarea">${destinations
                            .join(' ')
                            .trim()}</textarea>
                    </div>
                    <div>
                        <label for="raOriginCoords">${twSDK.tt(
                            'Origin Villages'
                        )}: ${origins.length}</label>
                        <textarea id="raOriginCoords" class="ra-textarea">${origins
                            .join(' ')
                            .trim()}</textarea>
                    </div>
                </div>
    ...
    // Helper: Prepare table of content
    function buildTable(array, type) {
        let typeOfCoords = '';
        if (type === 'destination') {
            typeOfCoords = twSDK.tt('Destination Villages');
        } else if (type === 'origin') {
            typeOfCoords = twSDK.tt('Origin Villages');
        } else {
            typeOfCoords = '';
        }
        ...
        array.forEach((item) => {
            const { coord, count } = item;
            const [x, y] = coord.split('|');

            let link = ``;
            if (game_data.player.sitter > 0) {
                link = `/game.php?t=${game_data.player.id}&screen=map&x=${x}&y=${y}&beacon#${x};${y}`;
            } else {
                link = `/game.php?screen=map&x=${x}&y=${y}&beacon#${x};${y}`;
            }
    ...
    // Helper: Collect commands information from commands overview
    function collectCommands() {
        try {
            const commands = [];
            jQuery('#commands_table tbody tr.nowrap').each(function () {
                const destinationVillage =
                    jQuery(this)
                        .find('td:eq(0)')
                        .text()
                        ?.match(twSDK.coordsRegex) ?? '';
                const originVillage = jQuery(this)
                    .find('td:eq(1)')
                    .text()
                    .match(twSDK.coordsRegex);
                const commandType = jQuery(this)
                    .find('td:eq(0) span:eq(0)')
                    .attr('data-command-type');
                const commandId = jQuery(this)
                    .find('td:eq(0) span:eq(0)')
                    .attr('data-command-id');
                const commandSubType = jQuery(this)
                    .find('td:eq(0) span:eq(0) img')
                    .attr('src')
                    .split('/')
                    .pop()
                    .split('#')[0]
                ...
                if (destinationVillage) {
                    commands.push({
                        destination: destinationVillage[0],
                        origin: originVillage[0],
            ...
            UI.ErrorMessage(
                twSDK.tt('There was an error collecting command informations!')
            );
    ...
    function parseCommands(commands) {
        ...
        const originCoords = [];
        const commandTypes = [];
        const commandSubTypes = [];
        const commandsWithNoblesArray = [];

        commands.forEach((item) => {
            const { destination, origin, type, subType, containsNoble } = item;
            destinationCoords.push(destination);
            originCoords.push(origin);
            commandTypes.push(type);
            commandSubTypes.push(subType);
            commandsWithNoblesArray.push(containsNoble);
        });

        // get combinations of destination and origin counts
        const destinationVillagesCount =
            twSDK.frequencyCounter(destinationCoords);
        const originVillagesCount = twSDK.frequencyCounter(originCoords);

        // get combinations of commands based on type
        const commandTypesCount = twSDK.frequencyCounter(commandTypes);
        const commandSubTypesCount = twSDK.frequencyCounter(commandSubTypes);

        // get arrays of unique destinations and origins
        const destinations = Object.keys(destinationVillagesCount);
        const origins = Object.keys(originVillagesCount);

        // find commands with nobles
        const commandsWithNobles = commandsWithNoblesArray.filter(
            (command) =>
                command === 'snob.webp' || command === 'return_snob.webp'
        );

        // convert object to array
        let destinationVillagesCountArray = convertObjectToArray(
            destinationVillagesCount
        );
        let originVillagesCountArray =
            convertObjectToArray(originVillagesCount);

        // sort arrays by count
        destinationVillagesCountArray = twSDK.sortArrayOfObjectsByKey(
            destinationVillagesCountArray,
            'count'
        );
        originVillagesCountArray = twSDK.sortArrayOfObjectsByKey(
            originVillagesCountArray,
            'count'
        );
