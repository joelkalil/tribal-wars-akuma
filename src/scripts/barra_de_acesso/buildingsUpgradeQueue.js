/*
 * Script Name: Buildings Upgrade Queue
...
if (typeof BUILDINGS_ORDER === 'undefined') BUILDINGS_ORDER = [];

// Script Config
var scriptConfig = {
    scriptData: {
        prefix: 'buildingsUpgradeQueue',
        name: 'Buildings Upgrade Queue',
        version: 'v1.0',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/buildings-upgrade-queue.292454/',
    },
    translations: {
        en_DK: {
            'Buildings Upgrade Queue': 'Buildings Upgrade Queue',
            Help: 'Help',
            'Redirecting...': 'Redirecting...',
            'Buildings upgrade order has not been defined!':
                'Buildings upgrade order has not been defined!',
            'Generated Script': 'Generated Script',
            'Add to Quick-bar': 'Add to Quick-bar',
            'Script was added on Quick-bar!': 'Script was added on Quick-bar!',
            'All buildings are upgraded!': 'All buildings are upgraded!',
        },
    },
    allowedMarkets: [],
    allowedScreens: ['main'],
...
(async function () {
    ...
    const gameDataBuildings = Object.keys(game_data.village.buildings);

    // Entry Point
    (function () {
        // check that we are on the right screen
        if (gameScreen !== 'main' && gameScreen !== 'am_village') {
            UI.InfoMessage(twSDK.tt('Redirecting...'));
            twSDK.redirectTo('main');
        } else {
            if (gameScreen === 'am_village' && gameMode === 'queue') {
                initQueueScreen();
            } else {
                initMainScreen();
            }
    ...
    // Initialize main script logic on main screen (village headquarter)
    function initMainScreen() {
        if (BUILDINGS_ORDER.length === 0) {
            UI.ErrorMessage(
                twSDK.tt('Buildings upgrade order has not been defined!')
            );
        } else {
            const nextBuildingToUpgrade = getNextBuildingToUpgrade();
            if (nextBuildingToUpgrade) {
                jQuery(`#main_buildrow_${nextBuildingToUpgrade} td`).css(
                    'background-color',
                    'yellow'
                );
                jQuery('html,body').animate({
                    scrollTop:
                        jQuery(
                            `#main_buildrow_${nextBuildingToUpgrade}`
                        ).offset().top - 100,
                });
            } else {
                UI.SuccessMessage(twSDK.tt('All buildings are upgraded!'));
            }
        }
    }

    // Initialize main script logic on queue screen (AM building template edit mode)
    function initQueueScreen() {
        let buildingsQueue = [];

        jQuery('#template_queue li').each(function () {
            const building = jQuery(this)
                .find('a')
                .attr('href')
                .split('screen=')[1];
            const level = parseLevel(
                jQuery(this).find('.level_absolute').text().trim()
            );

            buildingsQueue.push(building + level);
        });

        let generatedScriptString = generateScript(buildingsQueue);
        let templateName = jQuery('h3:eq(0)').text().split(': ')[1];
                ...
                <div class="ra-mb15">
                    <table class="ra-table ra-table-v3" width="100%">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <textarea id="raGeneratedScript" readonly class="ra-textarea">${generatedScriptString}</textarea>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
                                <td>
                                    <a href="javascript:void(0);" class="btn" id="raAddToQuickbarBtn">${twSDK.tt(
                                        'Add to Quick-bar'
                                    )}</a>
                                </td>
                            </tr>
        ...
        twSDK.renderFixedWidget(
            ...
            'ra-buildings-upgrade-queue-widget'
        );

        jQuery('#raAddToQuickbarBtn').click(function () {
            twSDK.addScriptToQuickbar(
                templateName,
                generatedScriptString,
                () => {
                    UI.SuccessMessage(
                        twSDK.tt('Script was added on Quick-bar!')
                    );
                }
            );
        });
    ...
    // Helper: Generate script for quick-bar
    function generateScript(buildingsQueue) {
        return `javascript:var BUILDINGS_ORDER=${JSON.stringify(
            buildingsQueue
        )};$.getScript('https://raw.githubusercontent.com/joelkalil/tribal-wars-akuma/refs/heads/main/src/scripts/barra_de_acesso/buildingsUpgradeQueue.js');`;
    }

    // Helper: Get next building that needs to be upgraded
    function getNextBuildingToUpgrade() {
        const buildingLevels = getBuildingLevels();
        const buildingsUpgradeQueue = [];

        BUILDINGS_ORDER.forEach((queueBuilding) => {
            const queueBuildingName = parseName(queueBuilding);
            const queueBuildingLevel = parseLevel(queueBuilding);
            const buildingLevel = buildingLevels[queueBuildingName];

            if (buildingLevel) {
                const currentBuildingName = parseName(buildingLevel);
                const currentBuildingLevel = parseLevel(buildingLevel);
                if (
                    currentBuildingLevel <= queueBuildingLevel &&
                    queueBuildingName === currentBuildingName
                ) {
                    buildingsUpgradeQueue.push(queueBuilding);
                }
            }
        });

        const nextBuildingToUpgrade = buildingsUpgradeQueue[0];
        if (nextBuildingToUpgrade) {
            return parseName(nextBuildingToUpgrade);
        }
    }

    // Helper: Parse building name
    function parseName(name) {
        return name.replace(/[0-9]/g, '');
    }

    // Helper: Parse building level
    function parseLevel(level) {
        return parseInt(level.replace(/\D/g, ''));
    }

    // Helper: Get village levels from the DOM
    function getBuildingLevels() {
        let buildingLevels = {};

        gameDataBuildings.forEach((buildingName) => {
            let element = document.querySelector(
                `a.btn-build[data-building="${buildingName}"]`
            );
            let value = element?.textContent?.slice(-2);

            if (element) {
                if (!isNaN(value)) {
                    buildingLevels[buildingName] = `${buildingName}${parseInt(
                        value
                    )}`;
                } else {
                    buildingLevels[buildingName] = `${buildingName}1`;
                }

                if (
                    document.querySelector(
                        `#main_buildrow_${buildingName} > td.inactive.center`
                    ) !== null
                ) {
                    buildingLevels[buildingName] = `${buildingName}30`;
                }
            }
        });
