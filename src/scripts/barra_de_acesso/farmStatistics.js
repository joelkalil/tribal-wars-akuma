...
var scriptConfig = {
    scriptData: {
        prefix: 'farmStatistics',
        name: 'Farm Statistics',
        version: 'v1.0.2-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/farm-statistics.291556/',
    },
    translations: {
        en_DK: {
            'Farm Statistics': 'Farm Statistics',
            Help: 'Help',
            'Script is running ...': 'Script is running ...',
            'This script requires a Discord WebHook to be provided!':
                'This script requires a Discord WebHook to be provided!',
            'There was an error while fetching farming statistics!':
                'There was an error while fetching farming statistics!',
            'Server Time': 'Server Time',
            Farmed: 'Farmed',
            'Plundered Villages': 'Plundered Villages',
            Gathered: 'Gathered',
            'There was an error while sending data to Discord!':
                'There was an error while sending data to Discord!',
            'Average farmed resources per run':
                'Average farmed resources per run',
            'Estimated resources farmed for today':
                'Estimated resources farmed for today',
...
(async function () {
    ...
    // Entry point
    (async function () {
        UI.InfoMessage(twSDK.tt('Script is running ...'));

        fetchData((data) => {
            let [farmedAmount, plunderedAmount, gatheredAmount] = data;
            const { avgFarmedResources, estimatedFarmedResources } =
                doFarmingCalculations(farmedAmount, plunderedAmount);
    ...
    function buildUI({
        ...
        const content = `
                    ...
                    <table class="ra-table ra-table-v3" width="100%">
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    ${twSDK.formatAsNumber(farmedAmount)}
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <b>${twSDK.tt('Plundered Villages')}</b>
                                </td>
                                <td>
                                    ${twSDK.formatAsNumber(plunderedAmount)}
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <b>${twSDK.tt('Gathered')}</b>
                                </td>
                                <td>
                                    ${twSDK.formatAsNumber(gatheredAmount)}
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <b>${twSDK.tt('Total Resources')}</b>
                                </td>
                                <td>
                                    ${twSDK.formatAsNumber(
                                        +gatheredAmount + +farmedAmount
                                    )}
                                </td>
                            </tr>
                            <tr>
                ...
                <div class="ra-mb15">
                    <a href="javascript:void(0);" id="raBtnSendToDiscord" class="btn">
    ...
    // Action Handler: Handle sending to discord
    function handleSendToDiscord({
        ...
        jQuery('#raBtnSendToDiscord').on('click', function (e) {
            e.preventDefault();
                ...
                embeds: [
                    {
                        ...
                        fields: [
                            {
                                name: twSDK.tt('Server Time'),
                                value: twSDK.getServerDateTime(),
                                inline: false,
                            },
                            {
                                name: twSDK.tt('Farmed'),
                                value: twSDK.formatAsNumber(farmedAmount),
                                inline: true,
                            },
                            {
                                name: twSDK.tt('Gathered'),
                                value: twSDK.formatAsNumber(gatheredAmount),
                                inline: true,
                            },
                            {
                                name: twSDK.tt('Plundered Villages'),
                                value: twSDK.formatAsNumber(plunderedAmount),
                                inline: false,
                            },
                            {
                                name: twSDK.tt('Total Resources'),
                                value: twSDK.formatAsNumber(
                                    +gatheredAmount + +farmedAmount
                                ),
                                inline: false,
                            },
            ...
            $.ajax({
                ...
                .done(() => {
                    UI.SuccessMessage('Your farming statistics were sent!');
                })
                .fail((error) => {
                    UI.ErrorMessage(
                        twSDK.tt(
    ...
    // Helper: Fetch all required data
    function fetchData(callback) {
        const results = [];
        const dataToFetch = ['loot_res', 'loot_vil', 'scavenge'];

        dataToFetch.forEach((item, index) => {
            setTimeout(async () => {
                let data = await fetchDataFromInADay(item);
                results.push(data);

                if (index === dataToFetch.length - 1) {
                    callback(results);
                }
            }, index * 200);
        });
    ...
    function doFarmingCalculations(farmedAmount, plunderedAmount) {
        let currentTime = new Date(twSDK.getServerDateTimeObject());
        let currentHour = currentTime.getHours();
        let currentMinute = currentTime.getMinutes();

        let totalHoursRemaining = 24 - (currentHour + currentMinute / 60);
        let farmPerHour = farmedAmount / (currentHour + currentMinute / 60);
        let estimatedFarmedResources =
            +farmedAmount + +farmPerHour * totalHoursRemaining;

        let avgFarmedResources = parseInt(farmedAmount / plunderedAmount) || 0;

        return { avgFarmedResources, estimatedFarmedResources };
    }

    // Helper: Fetch statistic from an "In a Day ranking" page
    async function fetchDataFromInADay(url) {
        try {
            return await jQuery
                .get(
                    `${game_data.link_base_pure}ranking&mode=in_a_day&type=${url}`
                )
                .then((response) => {
