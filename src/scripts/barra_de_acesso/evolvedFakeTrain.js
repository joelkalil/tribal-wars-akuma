...
(async function () {
            ...
            if (unit !== 'militia' && unit !== 'knight') {
                allUnitsToggles += `
                        <a href="javascript:void(0);" class="btn ra-btn-set-all-units" data-unit="${unit}">
                            <span><img src="/graphic/unit/unit_${unit}.webp"></span>
                        </a>
                    `;
            }
        });
    ...
    function unitAutoBalancer(units) {
        ...
        $.each(units, function (k, unit_id) {
            var pool =
                avail[unit_id] -
                send[unit_id] -
                Place.confirmScreen.getDirtyUnitCount(unit_id);
            var pool_used = 0;

            var $inputs = $('#place_confirm_units').find(
                'input[name*=\\[' + unit_id + '\\]]:not(.dirty)'
            );
    ...
    function toggleCurrentActiveButtonState(currentButton) {
        jQuery('.ra-evolved-fake-train-body .btn').removeClass(
            'btn-confirm-yes'
        );
