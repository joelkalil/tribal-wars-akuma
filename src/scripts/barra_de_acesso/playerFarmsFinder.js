...
var scriptConfig = {
    scriptData: {
        prefix: 'playerFarmsFinder',
        name: 'Player Farms Finder',
        version: 'v1.3.1-akuma.1',
        author: 'RedAlert',
...
(async function () {
    ...
    function buildUI() {
        ...
        // Show popup
        const [barbsCoordsList, areaCoords, barbsCount, playerVillagesCount] =
            doPlayerBarbsCalculations(20);

        const scoutBarbsScript = generateScoutScript(barbsCoordsList);
        ...
        const content = `
                    ...
                    <table class="ra-table ra-table-v3" width="100%">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span id="barbsCount">${barbsCount}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <strong>${twSDK.tt('Area')}</strong> 
                                </td>
                                <td>
                                    <span id="areaCoords">${areaCoords}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <strong>${twSDK.tt('Radius')}</strong>
                                </td>
                                <td>
                ...
                <div class="ra-mb15">
                    <label class="ra-label" for="barbCoordsList">${twSDK.tt(
                        'Coordinates'
                    )}</label>
                    <textarea id="barbCoordsList" class="ra-textarea" readonly>${barbsCoordsList}</textarea>
                </div>
                <div class="ra-mb15">
                    <label class="ra-label" for="barbCoordsScript">${twSDK.tt(
                        'Scout Script'
                    )}</label>
        ...
        const customStyle = `
			    ...
			    .ra-input { display: block; width: 100%; height: auto; padding: 5px 0; }
		    `;

        twSDK.renderFixedWidget(
            content,
            scriptConfig.scriptData.prefix,
            'ra-player-farms-finder',
            customStyle
        );
    }

    // Action Handler: Calculate Barbs for Player onChange of select
    function handleUpdateFarms() {
        jQuery('#radius_choser').on('change', function (e) {
            const radius = parseInt(e.target.value);

            const [barbsCoordsList, areaCoords, barbsCount] =
                doPlayerBarbsCalculations(radius);

            const scoutBarbsScript = generateScoutScript(barbsCoordsList);

            jQuery('#barbsCount').text(barbsCount);
            jQuery('#areaCoords').text(areaCoords);
            jQuery('#barbCoordsList').val(barbsCoordsList);
            jQuery('#barbCoordsScript').val(scoutBarbsScript);
        });
    }

    // Helper: Calculate Barbs for Player on Load Time
    function doPlayerBarbsCalculations(radius) {
        ...
        const barbsInsidePlayerRadius = calculateBarbsInsideRadius(
            minX,
            minY,
            maxX,
            maxY
        ...
        const barbCoords = barbsInsidePlayerRadius.map(
            (barb) => barb[2] + '|' + barb[3]
        );
        const barbsCount = barbCoords.length;
        const barbsCoordsList = barbCoords.join(' ');
        const playerVillagesCount = playerVillages.length;
