/*
 * Script Name: localStorage Manager
...
var scriptConfig = {
    scriptData: {
        prefix: 'localStorageManager',
        name: 'localStorage Manager',
        version: 'v1.0.4-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/localstorage-manager.289684/',
    },
    translations: {
        en_DK: {
...
(async function () {
    ...
    if (typeof Storage === 'undefined') {
        UI.ErrorMessage(
            twSDK.tt('localStorage is not supported by your browser!')
        );
        return;
    }
    ...
    function buildUI() {
        ...
        const content = `
                <div class="ra-mb15">
                    <div class="ra-table-container" id="raLocalStorageItems">
                        ${table}
                    </div>
                </div>
            `;
        ...
        twSDK.renderFixedWidget(
            content,
            'raLocalStorageManager',
            'ra-local-storage-manager'
        );
    }

    // Action Handler: Delete localStorage item
    function handleLocalStorageItemActions() {
        jQuery('.ra-show-local-storage-item').on('click', function (e) {
            e.preventDefault();

            const selectedKey = jQuery(e.target).attr('data-key');

            const content = `
                    <div class="ra-popup-content ra-mb15">
                        <textarea readonly class="ra-textarea">${localStorage.getItem(
                            selectedKey
                        )}</textarea>
                    </div>
                `;

            Dialog.show('raLocalStorageContent', content);
        });

        jQuery('.btn-remove-local-storage-item').on('click', function (e) {
            e.preventDefault();

            const selectedKey = jQuery(e.target).attr('data-key');
            localStorage.removeItem(selectedKey);
            jQuery(e.target).parent().parent().fadeOut(200);
        });
    }

    // Helper: Prepare table of localStorage items
        ...
        localStorageKeys.forEach((key) => {
            if (
                ...
                table += `
                        <tr>
                            <td width="80%">
                                <a href="javascript:void(0);" class="ra-show-local-storage-item" data-key="${key}">
                                    ${key}
                                </a>
                            </td>
                            <td width="20%">
                                <a href="javascript:void(0);" class="btn btn-remove-local-storage-item" data-key="${key}">
                                    ${twSDK.tt('Delete')}
                                </a>
                            </td>
                        </tr>
