/*
 * Script Name: Redirector
 * Version: v1.3.4-akuma.1
...
var scriptConfig = {
    scriptData: {
        prefix: 'redirector',
        name: `Redirector`,
        version: 'v1.3.3-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/redirector.288171/',
    },
    translations: {
        en_DK: {
            ...
            'Mass Redirector': 'Mass Redirector',
            ...
            'Single Village': 'Single Village',
            'Village Overview': 'Village Overview',
            'Link has been set to redirect to':
                'Link has been set to redirect to',
            'Mass Open Links': 'Mass Open Links',
            'No screen has been selected!': 'No screen has been selected!',
            'There are no links to change on this overview!':
                'There are no links to change on this overview!',
            'There are no links to open on this overview!':
                'There are no links to open on this overview!',
        },
    },
    allowedMarkets: [],
...
(async function () {
    ...
    const baseUrl = game_data.link_base;
    ...
    function buildUI() {
        const villageBuildings = twSDK.getVillageBuildings();
        const singleRedirectLinks = getSingleRedirectLinks(villageBuildings);
        const massRedirector = buildMassRedirectorSelector();
        const content = prepareContent(singleRedirectLinks, massRedirector);

        const customStyle = `
                .ra-redirector-wrapper {
                    display: flex;
    ...
    // Action Handler: Handle mass redirect links
    function handleMassRedirectLinks() {
        jQuery('.ra-chosen-screen-btn').on('click', function (event) {
            event.preventDefault();

            const thereAreLinksToChange = jQuery('.overview_table tr').length;
            if (thereAreLinksToChange) {
                const chosenScreen = jQuery(this).attr('data-screen');
                const chosenScreenTitle = getScreenLabel(chosenScreen);

                jQuery('#raScreenSelect').val(chosenScreen);

                jQuery('.ra-chosen-screen-btn').removeClass('btn-active');
                jQuery(this).addClass('btn-active');

                jQuery('.overview_table tr:not(:first)').each(function () {
                    const currentVillageId = jQuery(this)
                        .find('.quickedit-vn')
                        .attr('data-id');
                    const linkElement = jQuery(this).find(
                        'td:eq(1) span a:eq(0)'
                    );

                    let newLink = `/game.php?village=${currentVillageId}&screen=${chosenScreen}`;
                    if (game_data.player.sitter > 0) {
                        newLink = `/game.php?village=${currentVillageId}&screen=${chosenScreen}&t=${game_data.player.id}`;
                    }
                ...
                UI.ErrorMessage(
                    twSDK.tt('There are no links to change on this overview!')
                );
            }
        });

        jQuery('#raMassOpenLinksBtn').on('click', function (event) {
            event.preventDefault();

            const thereAreLinksToChange = jQuery('.overview_table tr').length;
            if (thereAreLinksToChange) {
                const chosenScreen = jQuery(
                    '.ra-chosen-screen-btn.btn-active'
                ).attr('data-screen');

                if (chosenScreen && chosenScreen.length !== 0) {
                    const targetUrls = [];
                    ...
                    jQuery('.overview_table tr:not(:first)').each(function () {
                        ...
                        if (chosenScreen === 'info_village') {
                            newLink += `&id=${currentVillageId}`;
                        }

                        targetUrls.push(newLink);
                    });

                    targetUrls.forEach((url, i) => {
                        setTimeout(
                            () => window.open(url),
                            i * twSDK.delayBetweenRequests
                        );
                    });
                } else {
                    UI.ErrorMessage(twSDK.tt('No screen has been selected!'));
                }
            } else {
                UI.ErrorMessage(
                    twSDK.tt('There are no links to open on this overview!')
                );
            }
        });
    }
    ...
    function prepareContent(singleRedirectLinks, massRedirector) {
        const gameScreen = twSDK.getParameterByName('screen');
        const gameMode = twSDK.getParameterByName('mode');

        const isProductionOverview =
            gameScreen === 'overview_villages' &&
            (gameMode === 'prod' || gameMode === 'combined');

        return `
                <div class="ra-single-redirector ra-mb15">
                    <div class="ra-redirector-wrapper">
                        ${singleRedirectLinks}
                    </div>
                </div>
                ${
                    isProductionOverview
                        ? `<div class="ra-mass-redirector ra-mb15">
                    ${massRedirector}
                </div>`
                        : ''
                }
    ...
    // Helper: Get single redirect links
    function getSingleRedirectLinks(villageBuildings) {
        let singleRedirectLinks = ``;

        villageBuildings.forEach((buildingLine) => {
            const { building } = buildingLine;

            singleRedirectLinks += `
                    <a href="${baseUrl}${building}" class="ra-redirector-btn" title="${getScreenLabel(
                building
            )}">
                        <img src="/graphic/buildings/${building}.webp">
                    </a>
                `;
        });

        return singleRedirectLinks;
    }

    // Helper: Build mass redirector selector
    function buildMassRedirectorSelector() {
        let redirectorSelector = `
			<input id="raScreenSelect" type="hidden">
		`;

        Object.keys(game_data.village.buildings).forEach((screen) => {
            redirectorSelector += `
                    <a href="${baseUrl}${screen}" class="ra-chosen-screen-btn" title="${getScreenLabel(
                screen
            )}" data-screen="${screen}">
                        <img src="/graphic/buildings/${screen}.webp">
                    </a>
                `;
        });

        redirectorSelector += `
                <a href="${baseUrl}${'map'}" class="ra-chosen-screen-btn" title="${twSDK.tt(
            'Map'
        )}" data-screen="map">
                    <img src="/graphic/icons/map2.webp">
                </a>
                <a href="${baseUrl}${'memo'}" class="ra-chosen-screen-btn" title="${twSDK.tt(
            'Notebook'
        )}" data-screen="memo">
                    <img src="/graphic/overview/note.webp">
                </a>
                <a href="${baseUrl}${'memo'}" class="ra-chosen-screen-btn" title="${twSDK.tt(
            'Single Village'
        )}" data-screen="info_village">
                <span class="icon header overview_village"></span>
                </a>
                <a href="${baseUrl}${'memo'}" class="ra-chosen-screen-btn" title="${twSDK.tt(
            'Village Overview'
        ...
        const massRedirectContent = `
                <h3>${twSDK.tt('Mass Redirector')}</h3>
                <div class="ra-redirector-wrapper ra-mb15">
                    ${redirectorSelector}
                </div>
                <a href="javascript:void(0);" id="raMassOpenLinksBtn" class="btn">
                    ${twSDK.tt('Mass Open Links')}
                </a>
            `;

        return gameScreen === 'overview_villages' ? massRedirectContent : '';
    }

    // Helper: Get screen label from screen key
    function getScreenLabel(key) {
        const screenLabel = {
            main: twSDK.tt('Headquarters'),
            barracks: twSDK.tt('Barracks'),
            stable: twSDK.tt('Stable'),
            garage: twSDK.tt('Workshop'),
            church: twSDK.tt('Church'),
            church_f: twSDK.tt('First church'),
            snob: twSDK.tt('Academy'),
            smith: twSDK.tt('Smithy'),
            place: twSDK.tt('Rally point'),
            statue: twSDK.tt('Statue'),
            market: twSDK.tt('Market'),
            wood: twSDK.tt('Timber camp'),
            stone: twSDK.tt('Clay pit'),
            iron: twSDK.tt('Iron mine'),
            farm: twSDK.tt('Farm'),
            storage: twSDK.tt('Warehouse'),
            hide: twSDK.tt('Hiding place'),
            wall: twSDK.tt('Wall'),
            watchtower: twSDK.tt('Watchtower'),
            map: twSDK.tt('Map'),
            memo: twSDK.tt('Notebook'),
            info_village: twSDK.tt('Single Village'),
            overview: twSDK.tt('Village Overview'),
        };

        if (screenLabel[key] !== undefined) {
            return screenLabel[key];
        } else {
            return '';
        }
    }
