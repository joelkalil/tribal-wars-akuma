...
(async function () {
    ...
    function initializeBacktimesPlannerReport() {
        const { distance, slowestUnitType, battleTime, totalRemainingUnits } =
            parseReportData();
    ...
    function handleMasterSelectionToggle() {
        jQuery('#raMasterSelectionToggle').on('click', function (e) {
            const isChecked = jQuery('#raMasterSelectionToggle').is(':checked');
        ...
        jQuery('#raAutomaticExportBtn').on('click', function (e) {
            ...
            const selectedIncomings = incomings
                .filter((incoming) => {
                    ...
                    return (
                        attackType !== 'attack_small.webp' &&
                        attackType !== 'attack.webp'
                    );
                })
    ...
    function findSlowestUnit(units) {
        let sortedUnitSpeeds = sortObjectByKey(worldUnitInfo.config, 'speed');
    ...
    function calculateReturnTime(reportData) {
        ...
        const formattedTravelTime = twSDK.secondsToHms(parseInt(travelTime));
    ...
    async function collectIncomingsList() {
        ...
        jQuery('#incomings_table tbody tr.nowrap').each((_, incoming) => {
            ...
            if (label !== twSDK.tt('Attack') && unit) {
                ...
                const distance = twSDK.calculateDistance(origin, destination);
        ...
        if (HIDE_GREENS) {
            incomings = incomings.filter(
                (incoming) => incoming.attackType !== 'attack_small.webp'
            );
        }
