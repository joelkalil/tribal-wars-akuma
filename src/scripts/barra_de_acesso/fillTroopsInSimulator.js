/*
 * Script Name: Fill Troops in Simulator
...
var scriptConfig = {
    scriptData: {
        prefix: 'fillTroopsInSimulator',
        name: 'Fill Troops in Simulator',
        version: 'v1.0.2-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/fill-troops-in-simulator.287256/',
    },
    translations: {
        en_DK: {
            'Fill Troops in Simulator': 'Fill Troops in Simulator',
            Help: 'Help',
            'Redirecting...': 'Redirecting...',
            'There was an error fetching troops stationed on your village!':
                'There was an error fetching troops stationed on your village!',
            'There was an error fetching incoming support troops!':
                'There was an error fetching incoming support troops!',
...
(async function () {
    ...
    if (isValidScreen && isValidMode) {
        initFillTroopsInSimulator();
    } else {
        UI.InfoMessage(twSDK.tt('Redirecting...'));
        twSDK.redirectTo('place&mode=sim');
        return;
    }

    // Initialize script business logic
    async function initFillTroopsInSimulator() {
        const villageId = twSDK.getParameterByName('village');
        const incomingSupport = await fetchIncomingSupport(villageId);
        const totalTroops = countTotalTroops(incomingSupport);

        if (DEBUG) {
            console.debug(`${scriptInfo} villageId`, villageId);
            console.debug(`${scriptInfo} incomingSupport`, incomingSupport);
            console.debug(`${scriptInfo} totalTroops`, totalTroops);
        }

        // Fill the current vills info first
        jQuery('#def_fill_select').val('current_village');
        jQuery('#def_fill_select').trigger('change');

        // Fill troops in simulator
        totalTroops.forEach((troop) => {
            const { unit, value } = troop;
            jQuery(`input[name="def_${unit}"]`).val(value);
        });

        // Update UI
        const troopsTable = buildTroopsTable(totalTroops);
        const totalTroopsString = encodeURIComponent(
        ...
        const content = `
                ...
                <div class="ra-mb15">
                    <a href="javascript:void(0)" class="btn btn-confirm-yes" id="raExportBBCodeBtn" data-troops="${totalTroopsString}">
        ...
        twSDK.renderFixedWidget(
            content,
            'raFillTroopsInSimulator',
            'ra-fill-troops-simulator'
        );

        // action handlers
        exportTroopsAsBBCode();
    }
    ...
    async function fetchIncomingSupport(villageId) {
        try {
            ...
            UI.ErrorMessage(
                twSDK.tt('There was an error fetching incomings information!')
            );
    ...
    function countTotalTroops(incomingSupport) {
        ...
        const currentVillageUnits =
            Simulator.Filler.templates.defender.current_village.unit_counts;
        const units = Object.keys(currentVillageUnits);

        units.forEach((unit) => {
            incomingSupport.forEach((item) => {
                mergedArray.push({
                    unit: unit,
                    value: +item[unit] + +currentVillageUnits[unit],
                });
            });
        });
    ...
    // Helper: Build total troops table
    function buildTroopsTable(troops) {
        let troopsTable = `<table class="ra-table" width="100%"><tbody>`;
        troops.forEach((troop) => {
            ...
            if (value > 0) {
                troopsTable += `
                        <tr>
                            <td class="ra-tal">
                                <img src="/graphic/unit/unit_${unit}.webp"> ${getUnitLabel(
                    unit
                )}
                            </td>
                            <td class="ra-tal">
    ...
    function getTroopsBBCode(troops) {
        const villageCoord = game_data.village.coord;
        let bbCode = ``;

        troops.forEach((troop) => {
            const { unit, value } = troop;
            if (value > 0) {
                bbCode += `[*][unit]${unit}[/unit] ${getUnitLabel(
                    unit
                )}[|][b]${twSDK.formatAsNumber(value)}[/b]\n`;
            }
        });

        return `[size=12] ${villageCoord} [/size]\n[table][**]${twSDK.tt(
            'Unit'
        )}[||]${twSDK.tt('Amount')}[/**]${bbCode}[/table]`;
    }

    // Action Handler: Export troops as BB Code
    function exportTroopsAsBBCode() {
        jQuery('#raExportBBCodeBtn').on('click', function (e) {
            e.preventDefault();
            let totalTroops = decodeURIComponent(
                jQuery(this).attr('data-troops')
            );
            totalTroops = JSON.parse(totalTroops);
            if (totalTroops.length) {
                const troopsBBCode = getTroopsBBCode(totalTroops);
                twSDK.copyToClipboard(troopsBBCode);
