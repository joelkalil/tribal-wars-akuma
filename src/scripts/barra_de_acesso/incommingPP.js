...
var scriptConfig = {
    scriptData: {
        prefix: 'incomingPP',
        name: 'Count Incoming PP',
        version: 'v1.0.6-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/count-incoming-pp.285687/',
    },
    translations: {
        en_DK: {
            'Count Incoming PP': 'Count Incoming PP',
            Help: 'Help',
            'Redirecting...': 'Redirecting...',
            'Total Incoming PP': 'Total Incoming PP',
            'Total PP including incoming': 'Total PP including incoming',
...
(async function () {
    ...
    const gameType = twSDK.getParameterByName('type');

    try {
        if (isValidScreen && isValidMode && gameType === 'inc') {
            const totalIncomingPP = doIncomingTransportsCalculations();
            const totalPP = parseInt(game_data.player.pp) + +totalIncomingPP;
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>${twSDK.formatAsNumber(
                                        totalIncomingPP
                                    )} <span class="icon header premium"></span></td>
                                </tr>
                                <tr>
                                    <td><b>${twSDK.tt(
                                        'Total PP including incoming'
                                    )}</b></td>
                                    <td>${twSDK.formatAsNumber(
                                        totalPP
                                    )} <span class="icon header premium"></span></td>
            ...
            twSDK.renderFixedWidget(content, 'raIncomingPP', 'ra-incoming-pp');
        } else {
            UI.InfoMessage(twSDK.tt('Redirecting...'));
            twSDK.redirectTo('overview_villages&mode=trader&type=inc');
        }
    } catch (error) {
    ...
    // Helper: Calculate Incoming Premium Points
    function doIncomingTransportsCalculations() {
        let totalIncomingPP = 0;

        jQuery('#trades_table tr')
            .not(':first')
            .not(':last')
            .each(function () {
                const currentIncomingType = jQuery(this)
                    .find('td:eq(8)')
                    .find('span')
                    .attr('class');
                if (currentIncomingType?.includes('premium')) {
                    let currentIncomingPPValue = jQuery(this)
                        .find('td:eq(8)')
                        .text();
                    currentIncomingPPValue = currentIncomingPPValue.replace(
                        /.(?=\d{3})/g,
                        ''
                    );
                    totalIncomingPP += parseInt(currentIncomingPPValue);
                }
            });

        return totalIncomingPP;
    }
})();
