...
var scriptConfig = {
    ...
    translations: {
        ...
        fr_FR: {
            ...
            'Script must be run from':
                'Le script doit être exécuté à partir de',
    ...
    allowedMarkets: [],
    allowedScreens: ['ranking'],
    allowedModes: ['player', 'ally', 'con_player', 'con_ally', null],
    isDebug: DEBUG,
    enableCountApi: true,
};
...
(async function () {
    ...
    let allowedGameScreens = ['ranking'];
    let allowedGameModes = ['player', 'ally', 'con_player', 'con_ally', null];
    let tables = {
    ...
    (function () {
        if (allowedGameScreens.includes(gameScreen)) {
            ...
            twSDK.redirectTo('ranking');
        }
    })();

    // Helper: Add graphs on each row
    function addRankingGraphs() {
        let gameMode = twSDK.getParameterByName('mode');

        if (gameMode === null) gameMode = 'player';
        let element = tables[gameMode];
        ...
        if (!$(element).hasClass('graphs-added')) {
            ...
            $(element)
                ...
                .each(function () {
                    ...
                    let url = location.protocol + location.host + link;

                    let id = twSDK.getParameterByName('id', url);
                    let [firstGraph, secondGraph] = generateGraphData(
                        id,
                        gameMode
                    );
            ...
            UI.ErrorMessage(twSDK.tt('Graphs already added!'));
        }
    }

    // Helper: Generate content
    function generateGraphData(id, mode) {
        let firstGraph, secondGraph;

        if (mode === 'player' || mode === 'con_player') {
            firstGraph = buildGraphImageUrl(id, 'playergraph', 'points');
            secondGraph = buildGraphImageUrl(id, 'playergraph', 'villages');
        }

        if (mode === 'ally' || mode === 'con_ally') {
            firstGraph = buildGraphImageUrl(id, 'tribegraph', 'points');
            secondGraph = buildGraphImageUrl(id, 'tribegraph', 'villages');
        }
