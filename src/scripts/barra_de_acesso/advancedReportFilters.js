/*
 * Script Name: Advanced Report Filters
 * Version: v1.0.7-akuma.1
...
var scriptConfig = {
    scriptData: {
        prefix: 'advancedReportFilters',
        name: 'Advanced Report Filters',
        version: 'v1.0.6-akuma.1',
        author: 'RedAlert',
        authorUrl: 'https://twscripts.dev/',
        helpLink:
            'https://forum.tribalwars.net/index.php?threads/advanced-report-filters.287778/',
    },
    translations: {
        en_DK: {
...
(async function () {
    ...
    if (isValidScreen && isValidMode) {
        ...
        const content = `
                <div class="ra-popup-content ra-mb15">
                    <button class="btn" onClick="javascript:twSDK.redirectTo('report&mode=attack');">
                        ${twSDK.tt('Attack Reports')}
                    </button>
                    <button class="btn" onClick="javascript:twSDK.redirectTo('report&mode=defense');">
                        ${twSDK.tt('Defense Reports')}
                    </button>
                </div>
            `;
        twSDK.renderFixedWidget(
            content,
            'raAdvancedReportFilters',
            'ra-advanced-report-filters'
        );
    }

    // Initialize main script logic
    async function initMain() {
        // check that there are reports
        if (jQuery("#report_list tr[class*='report-']").length === 0) {
            UI.ErrorMessage(twSDK.tt('No reports found!'));
            return;
        }
        ...
        // process data
        const attackingPlayersData = getPlayerDetailsByPlayerIds(
            attackers,
            players
        );
        const defendingPlayersData = getPlayerDetailsByPlayerIds(
            defenders,
            players
        );
        const attackingTribesData = getTribeDetailsByTribeIds(
            attackingTribes,
            tribes
        );
        const defendingTribesData = getTribeDetailsByTribeIds(
            defendingTribes,
            tribes
        );

        const sortedAttackingPlayersData = attackingPlayersData.sort(
            (a, b) => parseInt(a[5]) - parseInt(b[5])
        );
        const sortedDefendingPlayersData = defendingPlayersData.sort(
            (a, b) => parseInt(a[5]) - parseInt(b[5])
        );
        const sortedAttackingTribesData = attackingTribesData.sort(
            (a, b) => parseInt(a[7]) - parseInt(b[7])
        );
        const sortedDefendingTribesData = defendingTribesData.sort(
            (a, b) => parseInt(a[7]) - parseInt(b[7])
        );

        // build dropdowns
        const attackingPlayersDropdown = buildPlayersSelect(
            sortedAttackingPlayersData,
            'raAttackingPlayers'
        );
        const defendingPlayersDropdown = buildPlayersSelect(
            sortedDefendingPlayersData,
            'raDefendingPlayers'
        );
        const attackingTribesDropdown = buildTribesSelect(
            sortedAttackingTribesData,
            'raAttackingTribes'
        );
        const defendingTribesDropdown = buildTribesSelect(
            sortedDefendingTribesData,
            'raDefendingTribes'
        );
        ...
        const contentBody = `
                <div class="ra-mb15 ra-grid">
                    <fieldset class="ra-fieldset">
                        <legend>${twSDK.tt('Attacking Player')}</legend>
                        ${attackingPlayersDropdown}
                    </fieldset>
                    <fieldset class="ra-fieldset">
                        <legend>${twSDK.tt('Attacking Tribe')}</legend>
                        ${attackingTribesDropdown}
                    </fieldset>
                    <fieldset class="ra-fieldset">
                        <legend>${twSDK.tt('Defending Player')}</legend>
                        ${defendingPlayersDropdown}
                    </fieldset>
                    <fieldset class="ra-fieldset">
                        <legend>${twSDK.tt('Defending Tribe')}</legend>
                        ${defendingTribesDropdown}
                    </fieldset>
                </div>
                <div style="display:none;" id="raResultCoordinatesBox">
                    <label for="raResultCoordinates">${twSDK.tt(
                        'Result Coordinates'
                    )} <span id="raResultCoordinatesCount"></span></label>
                    <textarea class="ra-textarea" id="raResultCoordinates"></textarea>
                </div>
            `;
    ...
    // Event Handler: Handle on select change
    function handleOnChangeSelect(data) {
        jQuery('.ra-select').change(function () {
            switch (this.id) {
                case 'raAttackingPlayers':
                    selectPlayerReports(this.value, data, 'attacker');
                    break;
                case 'raDefendingPlayers':
                    selectPlayerReports(this.value, data, 'defender');
                    break;
                case 'raAttackingTribes':
                    selectTribeReports(this.value, data, 'attacker');
                    break;
                case 'raDefendingTribes':
                    selectTribeReports(this.value, data, 'defender');
                    break;
                default:
    ...
    // Helper: Select reports belonging to a specific player
    function selectPlayerReports(playerId, data, type) {
        let numberOfSelectedReports = 0;
        let chosenCoordinates = [];

        jQuery("#report_list tr[class*='report-']").each(function () {
            const reportName = jQuery(this)
                .find('.report-link')
                .find('.quickedit-label')
                .text()
                .trim();
            const { attackerCoords, defenderCoords } =
                getCoordinatesFromReportName(reportName);

            if (attackerCoords && defenderCoords) {
                const attackerId = getVillageOwnerByVillageCoord(
                    attackerCoords,
                    data.villages
                );
                const defenderId = getVillageOwnerByVillageCoord(
                    defenderCoords,
                    data.villages
                );

                if (type === 'attacker') {
                    if (parseInt(attackerId) === parseInt(playerId)) {
                        jQuery(this).show();
                        jQuery(this)
                            .find('td:eq(0) input[type=checkbox]')
                            .prop('checked', true);
                        numberOfSelectedReports++;
                        chosenCoordinates.push(attackerCoords);
                    } else {
                        jQuery(this).hide();
                        jQuery(this)
                            .find('td:eq(0) input[type=checkbox]')
                ...
                if (type === 'defender') {
                    if (parseInt(defenderId) === parseInt(playerId)) {
                        jQuery(this).show();
                        jQuery(this)
                            .find('td:eq(0) input[type=checkbox]')
                            .prop('checked', true);
                        chosenCoordinates.push(defenderCoords);
                        numberOfSelectedReports++;
                    } else {
                        jQuery(this).hide();
                        jQuery(this)
        ...
        chosenCoordinates = [...new Set(chosenCoordinates)];

        if (numberOfSelectedReports > 0) {
            const message = `${numberOfSelectedReports} ${twSDK.tt(
                'reports have been selected!'
            )}`;
            UI.SuccessMessage(message);
            jQuery('#raResultCoordinatesBox').show();
            jQuery('#raResultCoordinates').val(
                chosenCoordinates.join(' ').trim()
            );
            jQuery('#raResultCoordinatesCount').text(chosenCoordinates.length);
            jQuery('.selectAll').prop('disabled', true);
        } else {
            UI.ErrorMessage(twSDK.tt('No reports matched the criteria!'));
            jQuery('#raResultCoordinatesBox').hide();
            jQuery('#raResultCoordinates').val('');
            jQuery('#raResultCoordinatesCount').text('');
            jQuery('.selectAll').prop('disabled', false);
        }
    }

    // Helper: Select reports belonging to a specific tribe
    function selectTribeReports(tribeId, data, type) {
        let numberOfSelectedReports = 0;
        let chosenCoordinates = [];
        ...
        jQuery("#report_list tr[class*='report-']").each(function () {
            ...
            if (attackerCoords) {
                ...
                const attackerTribeId = getPlayerTribeByPlayerId(
                    attackerId,
                    data.players
                );
                const defenderTribeId = getPlayerTribeByPlayerId(
                    defenderId,
                    data.players
                );

                if (type === 'attacker') {
                    if (parseInt(attackerTribeId) === parseInt(tribeId)) {
                        jQuery(this).show();
                        jQuery(this)
                            .find('td:eq(0) input[type=checkbox]')
                            .prop('checked', true);
                ...
                if (type === 'defender') {
                    if (parseInt(defenderTribeId) === parseInt(tribeId)) {
                        jQuery(this).show();
                        jQuery(this)
                            .find('td:eq(0) input[type=checkbox]')
                            .prop('checked', true);
    ...
    // Helper: Get village owner by village coordinate
    function getVillageOwnerByVillageCoord(coord, villages) {
        let ownerID = 0;
        villages.forEach((village) => {
            const villageCoordinate = village[2] + '|' + village[3];
            if (villageCoordinate === coord) {
                ownerID = parseInt(village[4]);
            }
        });
        return ownerID;
    }

    // Helper: Get player details by providing a list of player Ids
    function getPlayerDetailsByPlayerIds(playerIds, players) {
        const playersData = [];
        players.forEach((player) => {
            if (playerIds.includes(parseInt(player[0]))) {
    ...
    // Helper: Get player details by providing a list of player Ids
    function getTribeDetailsByTribeIds(tribeIds, tribes) {
        const tribesData = [];
        tribes.forEach((tribe) => {
            if (tribeIds.includes(parseInt(tribe[0]))) {
                tribesData.push(tribe);
            }
        });
        return tribesData;
    }

    // Helper: Get tribe by player Id
    function getPlayerTribeByPlayerId(playerId, players) {
        let tribeId = 0;
        players.forEach((player) => {
            if (parseInt(player[0]) === parseInt(playerId)) {
                tribeId = parseInt(player[2]);
            }
        });
        return tribeId;
    }

    // Helper: Get coordinates from a report name
    function getCoordinatesFromReportName(reportName) {
        const parts = reportName.match(/\d+\|\d+/g);
        const attackerCoords = parts[0];
        const defenderCoords = parts.pop();
        return { attackerCoords, defenderCoords };
    }

    // Helper: Build players select dropdown
    function buildPlayersSelect(array, entity) {
        let dropdown = `<select class="ra-select" id="${entity}"><option selected disabled>${twSDK.tt(
            '--- choose player ---'
        )}</option>`;
        if (entity !== 'raAttackingPlayers') {
            dropdown += `<option value="0">${twSDK.tt(
                '--- Barbarian ---'
            )}</option>`;
        }
        array.forEach((item) => {
            if (item[0].length !== 0) {
                ...
                dropdown += `<option value="${id}">${cleanPlayerName}</option>`;
            }
        });
        dropdown += '</select>';
        return dropdown;
    ...
    // Helper: Build players select dropdown
    function buildTribesSelect(array, entity) {
        let dropdown = `<select class="ra-select" id="${entity}"><option selected disabled>${twSDK.tt(
            '--- choose tribe ---'
        )}</option>`;
        array.forEach((item) => {
            if (item[0].length !== 0) {
                const [id, _, tag] = item;
                const cleanTribeName = twSDK.cleanString(tag);
                dropdown += `<option value="${id}">${cleanTribeName}</option>`;
            }
        });
        dropdown += '</select>';
        return dropdown;
    ...
    // Helper: Parse and collect reports data
    function collectReportsData(worldData) {
        const { villages, players } = worldData;

        let attackers = [];
        let defenders = [];
        let attackingVillages = [];
        let defendingVillages = [];
        let attackingTribes = [];
        let defendingTribes = [];
        ...
        try {
            jQuery("#report_list tr[class*='report-']").each(function () {
                ...
                if (attackerCoords) {
                    ...
                    defenders.push(defenderId);
                    attackingVillages.push(attackerCoords);
                    defendingVillages.push(defenderCoords);
                    attackingTribes.push(attackerTribeId);
                    defendingTribes.push(defenderTribeId);
                }
            });
        } catch (error) {
            UI.ErrorMessage(
                twSDK.tt('There was an error parsing the reports!')
            );
            console.error(`${scriptInfo} Error: ${error}`);
        }

        // keep only unique values on the arrays
        attackers = [...new Set(attackers)];
        defenders = [...new Set(defenders)];
        attackingVillages = [...new Set(attackingVillages)];
        defendingVillages = [...new Set(defendingVillages)];
        attackingTribes = [...new Set(attackingTribes)];
        defendingTribes = [...new Set(defendingTribes)];
